...
![Advanced Screen Ruler v3.0 в действии](https://github.com/hardway777/ScreenRuler/blob/main/images/screenshotv4.5.png)
...
-   **Холст и захват фона**: Работайте со статичным изображением вашего экрана.
    ...
    -   **Два режима перетаскивания**:
        ...
        -   **Перемещение "вьюпорта" (Shift + Drag)**: Перемещает только окно над статичным холстом, как будто это камера.
...
-   **Полноценное управление сессиями**: Никогда не теряйте свою работу.
    -   **Сохранить/Загрузить сессию**: Сохраняйте все нарисованные фигуры, положение окна, размер, смещение холста и настройки калибровки в единый файл `.sez` (Session Zipped). Захваченный фон также включается в файл сохранения.
...
-   **Улучшения и исправления:**
    -   Исправлено рисование меток.
	
### v4.0
-   **Новые возможности:**
    -   Добавлен интерактивный режим перекалибровки (`R`) для установки масштаба по нарисованной линии.
    -   Добавлено сохранение/загрузка сессии в сжатый файл `.sez`, включая захваченное фоновое изображение.
    -   Приложение теперь может открывать файлы `.sez`, переданные через командную строку.
-   **Улучшения и исправления:**
    -   Переработана логика перетаскивания мышью для интуитивного панорамирования холста и "вьюпорта".
    -   Улучшен инструмент "Сетка" с умным размером по умолчанию и более плавной, пропорциональной настройкой.
    -   Улучшено позиционирование текста для всех фигур для лучшей читаемости.
    -   Исправлена ошибка, из-за которой текст состояния мог накладываться на шкалу линейки.
	
### v3.1
-   **Новые возможности:**
    -   Добавлена блокировка курсора удержанием `Ctrl` для точного позиционирования.

### v3.0 - The Precision CAD Update
Эта версия превратила линейку в полноценный 2D-инструмент для измерений и аннотаций с панорамируемым холстом, несколькими режимами рисования и продвинутыми функциями точности.
...
-   **Улучшения и исправления:**
    -   **Продвинутое управление мышью**: Переработана логика мыши для различения кликов и перетаскиваний.
    -   **Контекстные настройки**: Колесо мыши теперь динамически настраивает значения.
    -   **Улучшенная читаемость**: Весь текст теперь имеет высококонтрастную тень.
    -   **Улучшения UX**: Добавлено физическое прилипание курсора.
...
-   **Улучшения и исправления:**
    -   Добавлены высококонтрастные обводки для измерительных линий и текста.
    -   Горизонтальные и вертикальные оси были окрашены в разные цвета.
    -   Улучшена точность измерений.
